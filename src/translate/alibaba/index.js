...
async function alibabaTranslate(text, appId, secretKey, version = 'general', scene = 'general', from, to, original) {
	...
	const bodyMd5 = window.services.MD5Base64Encode(JSON.stringify(data));

	// 生成唯一随机值
	const uuid = window.services.uuid();
	...
	// 2. 计算 HMAC-SHA1
	const signature = window.services.HMACSha1(stringToSign, secretKey);
	...
	try {
		const res = await window.services.send(url, data, null, headers);
		console.log(res);
		
		if(res.Code !== '200') {
			const msg = errorCode[res.Code] || '未知错误，请稍后再试或联系管理员处理';
			return Promise.reject(new ErrorMessage('alibaba', msg));
		}
		const response = {
			name: 'alibaba',
